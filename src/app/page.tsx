    ...
    <main className="web-container py-8 md:py-12">
      ...
      {!searchQuery && selectedCategory === "All" && (
        <section className="mb-12 rounded-[2.5rem] bg-black p-8 text-center text-white md:p-16">
          ...
          <p className="mx-auto max-w-xl text-sm font-medium text-white/60 md:text-base">
      ...
      <section className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold tracking-tight text-zinc-900 md:text-2xl">Browse Categories</h2>
          {(searchQuery || selectedCategory !== "All") && (
            <button 
              onClick={() => {
                setSearchQuery("");
              ...
              className="text-xs font-bold text-[#12D16E] hover:underline uppercase tracking-wider"
            >
              Clear all
            </button>
          )}
        ...
        <div className="flex overflow-x-auto gap-2 pb-4 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0 md:flex-wrap md:pb-0">
          <button
            onClick={() => setSelectedCategory("All")}
            className={`whitespace-nowrap rounded-xl px-6 py-3 text-xs font-bold uppercase tracking-widest transition-all shrink-0 ${
              selectedCategory === "All"
                ? "bg-[#12D16E] text-white shadow-lg shadow-[#12D16E]/20"
            ...
            {filteredProducts.map((product) => (
              ...
              >
                <div className="relative aspect-square">
                  ...
                  />
                  <div className="absolute bottom-2 left-2 rounded-lg bg-white/90 px-2 py-1 text-xs font-bold backdrop-blur-sm">
                    {product.location}
                  </div>
                </div>
                <div className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-zinc-50 pt-3">
                    ...
                    <span className="rounded-md bg-zinc-100 px-1.5 py-0.5 text-xs font-semibold uppercase">
                      {product.unit}
                    </span>
                  </div>
                </div>
