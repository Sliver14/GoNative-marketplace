...
export default function MessagesPage() {
  ...
  return (
    <main className="web-container flex h-[calc(100vh-80px)] gap-6 py-6">
      {/* Left Side: Chat List Management */}
      <aside className="flex w-full md:w-96 flex-col rounded-[2.5rem] border border-zinc-100 bg-white shadow-sm overflow-hidden">
        <div className="p-6 border-b border-zinc-50">
          <h1 className="text-2xl font-black tracking-tight">Messages</h1>
          
          {/* Search Bar */}
          <div className="mt-6 flex items-center gap-3 rounded-2xl bg-zinc-50 px-4 py-3 border border-transparent focus-within:border-zinc-200 transition-all">
          ...
          <div className="mt-6 flex gap-2">
            <button 
              ...
              className={`rounded-full px-6 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all ${filter === "all" ? "bg-black text-white shadow-lg" : "text-zinc-400 hover:text-black hover:bg-zinc-50"}`}
            >
              All
            </button>
            <button 
              onClick={() => setFilter("unread")}
              className={`rounded-full px-6 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all ${filter === "unread" ? "bg-black text-white shadow-lg" : "text-zinc-400 hover:text-black hover:bg-zinc-50"}`}
            >
              Unread
            </button>
          </div>
          ...
          {conversations.map((chat) => (
            ...
            >
              <div className="relative shrink-0">
                <img src={chat.avatar} className="h-14 w-14 rounded-full object-cover shadow-sm" />
                {chat.unread > 0 && (
                  <div className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#12D16E] text-[10px] font-black text-[#FFD700] border-2 border-white">
                    {chat.unread}
                  </div>
                )}
              </div>
              <div className="flex-1 text-left min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <h4 className="font-bold text-zinc-900 truncate">{chat.name}</h4>
                  <span className="text-[10px] font-medium text-zinc-400 shrink-0">{chat.time}</span>
                </div>
                <p className="mt-1 line-clamp-1 text-xs font-medium text-zinc-500">{chat.lastMessage}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <section className="hidden md:flex flex-1 flex-col rounded-[2.5rem] border border-zinc-100 bg-white shadow-sm overflow-hidden">
        {activeChat ? (
          <>
            <header className="flex items-center justify-between border-b border-zinc-50 p-6">
              <div className="flex items-center gap-4">
                <img src={activeChat.avatar} className="h-12 w-12 rounded-full object-cover shadow-sm" />
                <div>
                  <h3 className="font-bold text-zinc-900">{activeChat.name}</h3>
                  <p className="text-[10px] font-black text-[#12D16E] uppercase tracking-[0.1em]">Online</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto bg-zinc-50/30 p-8 space-y-6">
              <div className="flex justify-center">
                <span className="rounded-full bg-white px-4 py-1.5 text-[10px] font-black text-zinc-400 shadow-sm uppercase tracking-widest border border-zinc-100">Today</span>
              </div>
              ...
              <div className="flex max-w-[70%] flex-col gap-2">
                ...
                <span className="text-[10px] font-bold text-zinc-400 ml-2">09:41 AM</span>
              </div>
            ...
            <footer className="p-6 border-t border-zinc-50 bg-white">
              <div className="flex items-center gap-4 rounded-[1.5rem] bg-zinc-50 p-2 pl-6 pr-2 focus-within:ring-2 focus-within:ring-[#12D16E]/20 transition-all border border-transparent focus-within:border-zinc-200">
                ...
                />
                <button className="grid h-12 w-12 place-items-center rounded-[1.2rem] bg-[#12D16E] text-[#FFD700] shadow-lg shadow-[#12D16E]/20 hover:scale-105 active:scale-95 transition-all">
